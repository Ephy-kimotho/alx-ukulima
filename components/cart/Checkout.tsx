...
import { CheckoutInitialValues } from "@/interfaces";
...
function Checkout({ toggleCheckout }: { toggleCheckout(): void }) {
  const handlePayment = (
    values: CheckoutInitialValues,
    action: FormikHelpers<CheckoutInitialValues>
  ) => {
    console.log(values);
    action.resetForm();
  };
  ...
  return (
    <section className="fixed z-[999] flex items-center justify-center  inset-0 inset-shadow-overlay">
      <div className="w-[90%] max-w-2xl   mx-auto bg-white rounded-2xl p-4">
        <div className="mb-5">
          <Button action={toggleCheckout}>
            <X size={30} color="#ED2939" />
          </Button>
          <h3 className="text-lime-green font-bold text-2xl">
        ...
        >
          <Form>
            ...
            <div className="flex justify-between mb-5">
              <AuthButton moreStyles="rounded-4xl py-2 px-8 md:px-16">
                Confirm
              </AuthButton>
              <Button
                action={toggleCheckout}
                moreStyles="bg-[#E54040] hover:bg-imperial-red border-2 border-[#E54040] rounded-4xl text-white font-bold py-2 px-10 md:px-16"
              >
                Cancel
              </Button>
            </div>
